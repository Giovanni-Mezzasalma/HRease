...
document.addEventListener('DOMContentLoaded', function() {
  ...
  refreshBtn.addEventListener('click', () => {
    state.currentPage = 1;
    restartAutoRefresh(); // Riabilita l'aggiornamento automatico quando si fa refresh manuale
    fetchLogs();
  });
  ...
  sourceSelect.addEventListener('change', () => {
    ...
    state.currentPage = 1;
    restartAutoRefresh(); // Riabilita l'aggiornamento automatico al cambio sorgente
    fetchLogs();
  });
  
  levelSelect.addEventListener('change', () => {
    state.currentPage = 1;
    // Disabilita l'aggiornamento automatico quando si applicano filtri
    pauseAutoRefreshForFiltering();
    fetchLogs();
  });
  
  searchInput.addEventListener('input', debounce(() => {
    state.currentPage = 1;
    // Disabilita l'aggiornamento automatico quando si cerca
    pauseAutoRefreshForFiltering();
    fetchLogs();
  }, 500));
  ...
  /**
   * Riavvia l'aggiornamento automatico se abilitato
   */
  ...
  /**
   * Mette in pausa l'aggiornamento automatico quando si applicano filtri
   */
  function pauseAutoRefreshForFiltering() {
    if (state.autoRefresh) {
      clearInterval(state.refreshInterval);
      ...
      refreshStatus.classList.remove('auto-refresh-active');
      refreshStatus.classList.add('auto-refresh-paused');
      refreshText.textContent = 'Auto-refresh paused (filtering)';
      toggleRefreshBtn.innerHTML = '<i class="fas fa-play"></i>';
    }
  }
